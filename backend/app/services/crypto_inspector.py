...
def scan_tls(hostname: str, port: int = 443, timeout: int = 15) -> dict:
    ...
    Uses Python's ssl module first, then falls back to OpenSSL s_client
    when stdlib probing fails to negotiate TLS data.
    ...
    # Primary: stdlib ssl module
    try:
        result = _scan_with_stdlib(hostname, port, timeout)
        if result.get("negotiated_protocol") and not result.get("error"):
            return result
    except Exception as e:
        result["error"] = str(e)
        logger.warning(f"stdlib TLS scan failed for {hostname}:{port}: {e}")

    # Fallback: openssl s_client
    openssl_result = _scan_with_openssl(hostname, port, timeout=timeout)
    if openssl_result.get("negotiated_protocol") and not openssl_result.get("error"):
        return openssl_result
    if openssl_result.get("certificate_chain_pem"):
        return openssl_result
    if openssl_result.get("raw_openssl_output"):
        return openssl_result
...
def _scan_with_openssl(hostname: str, port: int, timeout: int = 20, connect_host: str | None = None) -> dict:
    """Fallback TLS probe using openssl s_client when Python probes fail."""
    result = {
        "hostname": hostname,
        "port": port,
        "tls_versions_supported": [],
    ...
    try:
        ...
        result["raw_openssl_output"] = output

        protocol_match = re.search(r"Protocol:\s*([^\n\r]+)", output)
        if protocol_match:
            proto = protocol_match.group(1).strip()
            result["negotiated_protocol"] = proto
            result["tls_versions_supported"] = [proto]
        ...
        temp_key_match = re.search(r"Peer Temp Key:\s*([^,\n\r]+)", output)
        if temp_key_match:
            result["key_exchange"] = temp_key_match.group(1).strip()
            result["forward_secrecy"] = True
        elif result["negotiated_cipher"]:
            name = result["negotiated_cipher"].upper()
            result["forward_secrecy"] = ("ECDHE" in name) or ("DHE" in name) or name.startswith("TLS_")
            result["key_exchange"] = "X25519" if name.startswith("TLS_") else ("ECDHE" if "ECDHE" in name else "RSA")
        ...
        if pem_blocks:
            result["certificate_chain_pem"] = [f"{pem}\n".encode("utf-8") for pem in pem_blocks]
...
def inspect_asset(hostname: str, port: int = 443, pre_fetched_tls: dict = None, discovered_ip: str | None = None) -> dict:
    ...
    try:
        if _tls_prefetch_usable(pre_fetched_tls):
            ...
            if (not tls_result.get("negotiated_protocol") or tls_result.get("error")) and discovered_ip:
                ...
                except Exception as ip_retry_err:
                    logger.warning(
                        f"Direct-IP stdlib retry failed for {hostname}:{selected_port}: {ip_retry_err}",
                        extra={"hostname": hostname, "port": selected_port, "connect_host": discovered_ip},
                    )
            if (
                ...
                for candidate in (8443, 9443, 4443):
                    ...
                    if candidate_tls.get("negotiated_protocol") and not candidate_tls.get("error"):
                        ...
                        break
            if (not tls_result.get("negotiated_protocol")) or tls_result.get("error"):
                tls_result = _scan_with_openssl(
                    hostname,
                    selected_port,
                    timeout=20,
                    connect_host=discovered_ip,
                )
        fingerprint["tls"] = {
            ...
            "raw_openssl_output": tls_result.get("raw_openssl_output"),
        }
    ...
    tls_view = fingerprint.get("tls") or {}

    logger.info(
        f"Crypto inspection complete for {hostname}",
        extra={
            "hostname": hostname,
            "tls_versions": tls_view.get("versions_supported", []),
...
def save_crypto_results(
    ...
    saved_certs = []
    tls_data = fingerprint.get("tls") or {}
    ...
    for i, cert_data in enumerate(fingerprint.get("certificates", [])):
        ...
        cert = Certificate(
            ...
            negotiated_cipher=tls_data.get("negotiated_cipher"),
    ...
    if asset:
        negotiated_protocol = tls_data.get("negotiated_protocol")
        # Preserve discovery-phase HTTP TLS version if crypto scan couldn't negotiate one.
        if negotiated_protocol:
            asset.tls_version = negotiated_protocol
